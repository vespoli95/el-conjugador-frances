    ...
    <View>
      {tenseKeys.map(tenseKey => {
        ...
        if (typeof data === 'string') {
          return (
            <View key={tenseKey}>
              <Text style={[styles.tenseLabel, { color: isDark ? '#93c5fd' : '#1e3a8a' }]}>
                {tenseLabels[tenseKey] ?? tenseKey}
              </Text>
              <View style={[styles.row, { backgroundColor: isDark ? '#334155' : '#64748b' }]}>
        ...
        const isImperative = tenseKey === 'imperatif' || tenseKey === 'imperatifPasse';
          ...
          <View key={tenseKey}>
            ...
            {(data as string[]).map((value, i) => {
              ...
              return (
                <View
                  key={`${tenseKey}_${i}`}
                  style={[styles.row, { backgroundColor: isDark ? '#334155' : '#64748b' }]}
                >
                  <Text style={styles.rowText}>
                    {pronoun ? (
                      <Text>
                        {pronoun}: <Text style={styles.rowValueBold}>{value}</Text>
                      </Text>
                    ) : (
